// On modifie le code qui a été donné
 #include <SDL.h>
 #include <SDL_image.h>
 #include <SDL_ttf.h>
 #include <pthread.h>
 ...
 int jrejouant;	// Nombre de joueurs qui veulent rejouer
 
 int joueurCourant;	// Numéro du joueur courant
 int elimine[4];	// Liste de 0 et 1 : 1 si joueur éliminé, 0 sinon
 int jrestants;	// Nombre de joueurs non élimminés
 ...
 int main(int argc, char** argv) {
     ...
     sprintf(info, "Bonjour %s! Bienvenue au jeu Sherlock 13!", gName);
     ...
     while (!quit) {
         ...
         if (synchro == 1) {		
             ...
             switch (gbuffer[0]) {
                 ...
                 case 'E':
                     ...
                     sprintf(info, "%s is elimine! %s is innocent!", gNames[i], nbnoms[j]);
                     elimine[i] = 1;
                     jrestants--;
                     guiltGuess[j] = 1;
                     break;
                 ...
                 case 'R':
                     ...
                     jrejouant++;
                     sprintf(info, "%s wants to replay! (%d/4)", gNames[i], jrejouant);
                     break;
             }
             synchro = 0;
         }
         ...
         for (i = 0 ; i < 8 ; i++) {
            ...
            if (!surfaceMessage) {
                fprintf(stderr, "Erreur rendu texte [%s] : %s\n", nbobjets[i], TTF_GetError());
                continue; // on saute ce texte
            }
         ...
         for (i = 0 ; i < 4 ; i++)
             if (strlen(gNames[i]) > 0) {
                 // Nom joueur
                 SDL_Surface* surfaceMessage;
                 if (elimine[i]) {	// Si éliminé, en rouge
